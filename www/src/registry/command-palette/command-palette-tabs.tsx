...
import type { CommandPaletteSourceTab, CommandPaletteFilterTab } from "./command-palette-types";
...
function SourceTabItem({
  ...
  return (
    <button
      ...
      title={tab.label}
      className={cn(
        "relative flex items-center justify-center shrink-0 rounded-lg transition-all duration-150",
        tab.id === "all"
    ...
    >
      ...
      {tab.id !== "all" && tab.icon && !tab.logoSrc && (
      ...
      {isActive && (
        <motion.span
          ...
          className="absolute inset-0 rounded-lg border border-border/80 bg-muted/60"
          style={{ zIndex: -1 }}
