          ...
          <DashboardPanel className="space-y-4">
            ...
            <div className="rounded-lg border border-white/10 bg-background/50 p-4">
              ...
              <code className="block overflow-x-auto text-xs leading-6 text-muted-foreground">
                {`npx shadcn@latest add https://bevelui.com/api/r/pro/[system].json --auth ${primaryLicense.key}`}
              </code>
            </div>
          </DashboardPanel>
        </DashboardSection>
          ...
          {accessList.map(({ product, hasAccess, source }) => {
            const installCommand =
              hasAccess && product.registryPath
                ? product.tier === "FREE"
                  ? `bunx --bun shadcn@latest add https://bevelui.com/${product.registryPath}`
                  : `bunx --bun shadcn@latest add https://bevelui.com/api/${product.registryPath}`
                : null;
