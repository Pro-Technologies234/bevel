...
export function CropperCanvas({ className }: CropperCanvasProps) {
  const { src, imageSize, region, aspectRatio, config, setImageSize, setRegion } = useCropper();
  ...
  const transform: CanvasTransform | null =
    containerSize && imageSize
      ? computeTransform(containerSize.w, containerSize.h, imageSize.width, imageSize.height)
      : null;

  const cropBox = region && transform
    ? {
  ...
  function onSvgPointerDown(e: React.PointerEvent<SVGSVGElement>) {
    ...
    dragRef.current = { handle, startX: e.clientX, startY: e.clientY, startRegion: { ...region } };
  ...
  function onSvgPointerMove(e: React.PointerEvent<SVGSVGElement>) {
    if (dragRef.current) {
      ...
      setRegion(
        applyHandleDrag(startRegion, handle, dxImg, dyImg,
          imageSize.width, imageSize.height, aspectRatio, config.minSize),
      );
    } else if (cropBox) {
      const rect = e.currentTarget.getBoundingClientRect();
      const handle = hitTest(e.clientX - rect.left, e.clientY - rect.top, cropBox);
    ...
    >
      ...
      {cropBox && (
        ...
        >
            ...
            <g stroke="rgba(255,255,255,0.18)" strokeWidth={0.5}>
              {[1/3, 2/3].map((t) => (
                <React.Fragment key={t}>
                  <line
                    x1={cropBox.x + cropBox.w * t} y1={cropBox.y}
                    x2={cropBox.x + cropBox.w * t} y2={cropBox.y + cropBox.h}
                    className="stroke-white"
                  />
                  <line
                    x1={cropBox.x} y1={cropBox.y + cropBox.h * t}
                    x2={cropBox.x + cropBox.w} y2={cropBox.y + cropBox.h * t}
                    className="stroke-white"
                  />
                </React.Fragment>
              ))}
          ...
          {/* Crop border */}
          <rect
            x={cropBox.x} y={cropBox.y} width={cropBox.w} height={cropBox.h}
