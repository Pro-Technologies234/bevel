...
const SIZE = {
  sm: { card: "rounded-xl", aspect: "aspect-[4/3]", label: "text-xs", badge: "h-4 w-4", check: 10 },
  md: { card: "rounded-2xl", aspect: "aspect-[4/3]", label: "text-sm", badge: "h-5 w-5", check: 12 },
  lg: { card: "rounded-2xl", aspect: "aspect-[3/2]", label: "text-base", badge: "h-6 w-6", check: 14 },
} as const;
...
function DefaultCard<T>({
  ...
  isSelected,
  size = "md",
}: { option: CardSelectOption<T>; isSelected: boolean; size?: keyof typeof SIZE }) {
  const s = SIZE[size];
    ...
    <>
      ...
      <motion.div
        ...
        className={cn(
          ...
          s.card,
          iconOnly ? "flex items-center justify-center p-6 aspect-video" : s.aspect,
          isSelected
      ...
      >
        ...
        {iconOnly && (
          <div className={cn("text-foreground/60 transition-colors", isSelected && "text-foreground")}>
            {option.icon}
          </div>
        )}
        <AnimatePresence>
      ...
      <div className="flex flex-col items-center gap-2">
        <span className={cn("font-medium transition-colors duration-300", s.label, isSelected ? "text-foreground" : "text-foreground/40")}>
          {option.label}
        </span>
        {option.description && (
        ...
        <div className="h-1.5 w-1.5 rounded-full bg-foreground/10 overflow-hidden">
          <AnimatePresence>
            {isSelected && (
              <motion.div initial={{ y: 10 }} animate={{ y: 0 }} exit={{ y: 10 }} className="h-full w-full bg-foreground" />
            )}
          </AnimatePresence>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isSelected && (
          <motion.div
            ...
            className={cn("absolute top-2 right-2 z-10 flex items-center justify-center rounded-full bg-white text-black shadow-lg", s.badge)}
          >
            <IconCheck size={s.check} strokeWidth={4} />
          </motion.div>
        )}
      </AnimatePresence>
    </>
...
function ListCard<T>({
  ...
  return (
    <div className={cn(
      ...
      isSelected ? "border-primary/50 bg-white/5 outline-3" : "border-white/10 bg-white/5 opacity-80 hover:opacity-100",
    )}>
      {option.icon && (
        <div className={cn("text-foreground/60 shrink-0 transition-colors", isSelected && "text-foreground")}>
          {option.icon}
        </div>
      )}
      {typeof option.preview === "string" && (
        <img src={option.preview} alt={option.label} className="h-10 w-10 rounded-lg object-cover shrink-0" />
      )}
      <div className="flex-1 text-left">
        <div className={cn("font-medium", s.label, isSelected ? "text-foreground" : "text-foreground/70")}>
          {option.label}
        </div>
        {option.description && (
      ...
      <div className={cn(
        ...
        {isSelected && <IconCheck size={s.check} strokeWidth={3} />}
      </div>
    </div>
  );
}
...
export function CardSelect<T = string>({
  ...
  const [internalSingle, setInternalSingle] = useState<T | undefined>(
    !multiple && !isControlled ? (props as SingleUncontrolled<T>).defaultValue : undefined,
  );
  const [internalMulti, setInternalMulti] = useState<T[]>(
    multiple && !isControlled ? ((props as MultiUncontrolled<T>).defaultValue ?? []) : [],
  );

  const currentSingle = !multiple ? (isControlled ? (props as SingleControlled<T>).value : internalSingle) : undefined;
  const currentMulti = multiple ? (isControlled ? (props as MultiControlled<T>).value : internalMulti) : [];
  const max = multiple ? (props as MultiControlled<T> | MultiUncontrolled<T>).max : undefined;

  const isSelected = (val: T) => multiple ? currentMulti.includes(val) : currentSingle === val;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        layout === "scroll" && "flex flex-row gap-4 overflow-x-auto pb-2 snap-x",
        className,
      )}
    >
      {options.map((option, i) => {
        ...
        return (
          <motion.button
            variants={{ hidden: { y: 50, opacity: 0, scale: 0.58 }, show: { y: 0, opacity: 1, scale: 1 } }}
            ...
            className={cn(
              ...
              layout === "list" && "w-full",
              layout === "scroll" && cn("flex flex-col items-center gap-3 shrink-0 snap-start", scrollWidth),
              option.disabled && "cursor-not-allowed",
            )}
          >
            {renderCard ? renderCard(option, selected) : layout === "list"
