...
export function AIChatCodeBlock({ code, language = "text", filename }: { code: string; language?: string; filename?: string }) {
  const [copied, setCopied] = React.useState(false);
  const isPlain = PLAIN.has(language.toLowerCase());
  const trimmed = code.trim();

  const lines = React.useMemo(
    () => trimmed.split("\n").map((line) => ({ html: isPlain ? line : highlight(line || " "), raw: line })),
    [trimmed, isPlain],
  );
    ...
    <div className="rounded-lg border border-border bg-muted/50 overflow-hidden text-[12px] my-1 p-2.5 space-y-2.5">
      <div className="flex items-center justify-between px-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-mono text-muted-foreground/60 uppercase tracking-wide">{filename ?? language}</span>
        </div>
        <div>
          <AIChatAction variant="ghost" size="icon"><IconDownload size={16} /></AIChatAction>
      ...
      <pre className="overflow-x-auto px-4 py-3 font-mono leading-[1.65] rounded-lg bg-background border">
        {lines.map((line, i) => (
          <div key={i}>
            {isPlain ? <span className="text-foreground/80">{line.raw || " "}</span> : <span dangerouslySetInnerHTML={{ __html: line.html }} />}
          </div>
        ))}
      </pre>
    </div>
