...
export function AIChatWelcome() {
  ...
  const hasContent =
    config.welcomeTitle || config.welcomeMessage || config.starters?.length;
  if (!hasContent) return null;

  return (
    <div className="flex flex-col items-center justify-center gap-6 px-4 py-4">
      {(config.welcomeTitle || config.welcomeMessage) && (
        <div className="text-center max-w-md">
          {config.welcomeTitle && (
            <h2 className="text-4xl text-foreground mb-2 whitespace-pre-line tracking-tight">
              {config.welcomeTitle}
            </h2>
          )}
