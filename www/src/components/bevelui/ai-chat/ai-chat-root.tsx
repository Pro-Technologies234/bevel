...
import type {
  AIMessage, AIContentBlock, AIChatConfig,
  AIStreamCallbacks, AIChatContextValue,
} from "./types";

let _mid = 0;
function uid() { return `m_${++_mid}_${Date.now()}`; }
...
export interface AIChatRootProps {
   ...
   */
  onSend?: (text: string, callbacks: AIStreamCallbacks, attachments?: File[]) => Promise<void>;
...
export function AIChatRoot({
  ...
  function patchContent(id: string, fn: (c: AIContentBlock[]) => AIContentBlock[]) {
  ...
  async function sendMessage(text: string, attachments?: File[]) {
    if (isControlled) { onSendControlled?.(text, attachments); return; }
    if (!onSend) return;

    // User message
    setInternal(prev => [...prev, {
      id: uid(), role: "user", status: "done", timestamp: new Date(),
      content: [
        { type: "text", text },
        ...(attachments?.map<AIContentBlock>(f => ({
          type: "file", name: f.name, mimeType: f.type, size: f.size,
        })) ?? []),
      ],
    }]);
    ...
    const aid = uid();
    setInternal(prev => [...prev, {
      id: aid, role: "assistant", status: "streaming", model, timestamp: new Date(), content: [],
    }]);
    ...
    const cb: AIStreamCallbacks = {
      ...
      appendToken(token) {
        patchContent(aid, c => {
          ...
          if (last?.type === "text") { c[c.length - 1] = { type: "text", text: last.text + token }; }
      ...
      appendThinkingToken(token) {
        patchContent(aid, c => {
          const last = c[c.length - 1];
          if (last?.type === "thinking") { c[c.length - 1] = { ...last, text: last.text + token, isStreaming: true }; }
      ...
      updateBlock(index, p) {
        patchContent(aid, c => {
          c[index] = { ...c[index], ...p } as AIContentBlock; return c;
        });
      },
      ...
      setStatus(status) {
        ...
        if (status === "done" || status === "error") { setLoading(false); setStreaming(false); }
      },
    ...
    try {
      ...
      patch(aid, m => ({ ...m, status: isAbort ? "done" : "error", error: isAbort ? undefined : String(e) }));
      setLoading(false); setStreaming(false);
    }
  }
  ...
  function deleteMessage(id: string) { setInternal(p => p.filter(m => m.id !== id)); }
  function clearMessages()           { setInternal([]); }

  function toggleThinking(messageId: string, blockIndex: number) {
    patchContent(messageId, c => {
      const b = c[blockIndex];
      if (b?.type === "thinking") c[blockIndex] = { ...b, collapsed: !b.collapsed };
      return c;
    });
  }

  const ctx: AIChatContextValue = {
    messages, isLoading, isStreaming, model, config,
    setModel, sendMessage, stopGeneration,
    regenerate, deleteMessage, clearMessages, toggleThinking,
  };
