...
export function ChecklistWidget({ config }: { config: ChecklistConfig }) {
  const { steps, isOpen, toggle, close, progress, completedCount, requiredCount, isComplete } = useChecklist();

  const posClass = config.position === "bottom-left"
    ? "left-6 bottom-6"
    : "right-6 bottom-6";
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-80 rounded-2xl border border-border bg-card shadow-2xl shadow-black/40 overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-border/60">
              <div className="flex flex-col gap-0.5">
                ...
                {config.subtitle && (
                  <span className="text-[11px] text-muted-foreground/60">{config.subtitle}</span>
                )}
              </div>
              <button type="button" onClick={close} className="text-muted-foreground/40 hover:text-foreground transition-colors">
                <IconX size={14} />
              </button>
            </div>
            ...
            {isComplete ? (
              <div className="flex flex-col items-center gap-2 py-8 px-4">
                ...
                <p className="text-[13px] font-semibold text-foreground">You're all set</p>
