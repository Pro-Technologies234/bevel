...
export function SpotlightResultItem({ result }: { result: SpotlightResult }) {
  ...
  return (
    ...
    >
      <div className="w-9 h-9 rounded-md border border-border bg-muted/50 flex items-center justify-center shrink-0 overflow-hidden">
      ...
      {result.badge && (
        <span className="text-[10px] font-mono text-muted-foreground/30 shrink-0 ml-2">
          {result.badge}
        </span>
      )}
    </button>
